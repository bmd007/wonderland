...
import statefull.geofencing.faas.location.aggregate.config.TopicCreator;
...
public class KStreamAndKTableDefinitions {
    ...
    private static final Materialized<String, Mover, KeyValueStore<Bytes, byte[]>> IN_MEMORY_TEMP_KTABLE = Materialized
            .<String, Mover>as(Stores.inMemoryKeyValueStore(statefull.geofencing.faas.location.aggregate.config.Stores.MOVER_IN_MEMORY_STATE_STORE))
    ...
    public void configureStores() {
        ...
        var storeBuilder = new MoverStore.Builder(statefull.geofencing.faas.location.aggregate.config.Stores.MOVER_GLOBAL_STATE_STORE, Time.SYSTEM, repository);
        builder.addGlobalStore(storeBuilder,
                TopicCreator.storeTopicName(statefull.geofencing.faas.location.aggregate.config.Stores.MOVER_IN_MEMORY_STATE_STORE, applicationName),
                MOVER_CONSUMED, () -> new MoverProcessor(statefull.geofencing.faas.location.aggregate.config.Stores.MOVER_GLOBAL_STATE_STORE));
    }
